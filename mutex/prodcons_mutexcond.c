/*Práctica 4 - O problema do produtor-consumidor solucionado con mutexes e variables de condición*/
...
pthread_cond_t condp, condc; /*Variables de condición para cada tipo de fío*/
...
void *produtor(void *ptr){
	...
	for(int k = 0; k < a->ITER; k++){
		...
		while(conta == n) pthread_cond_wait(&condp, &the_mutex); /*Se o buffer está cheo bloquéase o fío*/
	...
	/*------------------------------------------------------------*/
		...
		pthread_cond_signal(&condc); /*Espértase a un consumidor bloqueado pola variable de condición*/	
...
void *consumidor(void *ptr){
	...
	for(int k = 0; k < a->ITER; k++){
		...
		pthread_mutex_lock(&the_mutex); /*Inténtase pechar o mutex*/
		while(conta == 0) pthread_cond_wait(&condc, &the_mutex); /*Se o buffer está baleiro, bloquéase o fío*/
		...
		pthread_cond_signal(&condp); /*Espértase a un produtor bloqueado pola variable de condición*/
		pthread_mutex_unlock(&the_mutex); /*Libérase o acceso ó buffer*/
...
int main(int argc, char **argv){
	...
	/*Inicialización de mutex, variables de condición e do buffer*/
