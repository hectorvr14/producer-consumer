/*Práctica 5 - Produtor FIFO con paso de mensaxes*/
...
void produtor(){
	...
	for(int i = 0; i < MAX_DATOS; i++){ /*Comézanse a producir os elementos*/
		...
		mq_send(almacen2, message, strlen(message), 1);/*Envíase a mensaxe, todas coa mesma prioridade - así, a cola de mensaxes extraerá a máis antiga, preservando o comportamento FIFO*/
		sleep(rand() % 4);
